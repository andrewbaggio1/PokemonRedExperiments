...
from .telemetry.writer import EventStreamWriter
...
def _sample_duration(options: Sequence[int]) -> int:
    if not options:
        return 1
    return int(np.random.choice(options))


def _extract_position(info) -> tuple[int, int, int]:
    coords = info.get("agent_coords") or (0, 0)
    x = int(coords[0]) if isinstance(coords, (tuple, list)) else 0
    y = int(coords[1]) if isinstance(coords, (tuple, list)) else 0
...
def actor_process(
    ...
    manager = EmulatorManager(cfg, rank)
    resume_state_path: Optional[str] = manager.resume_state_path() if getattr(cfg, "resume_from_snapshot", False) else None
    ...
    telemetry_writer: Optional[EventStreamWriter] = None
    telemetry_throttle: Optional[float] = None
    last_telemetry_time = 0.0
    if telemetry_cfg and getattr(telemetry_cfg, "enabled", False):
        events_path = os.path.join(cfg.run_dir, telemetry_cfg.events_file)
        telemetry_writer = EventStreamWriter(events_path, telemetry_cfg.flush_interval_s, telemetry_cfg.max_queue)
        telemetry_throttle = 1.0 / max(1e-3, telemetry_cfg.throttle_hz)
    action_duration_options = tuple(getattr(cfg, "action_duration_options", (1,)))
    # DDP rank (for unique file naming across jobs)
    try:
        ddp_rank = int(os.environ.get("RANK", "0"))
    except Exception:
        ddp_rank = 0
    rank_prefix = f"r{ddp_rank}_"
    ...
    policy = ClusterDQN((c, h, w), context_dim, env.action_space.n, cfg=cfg).to(device)
    ...
    try:
        while not stop_event.is_set():
            ...
            for t in range(max_steps):
                ...
                hidden = next_hidden
                if action_timer <= 0:
                    current_epsilon = epsilon_by_step(step_counter, cfg)
                    ...
                    if np.random.rand() < current_epsilon:
                        ...
                        if allowed_indices:
                            current_action = int(np.random.choice(allowed_indices))
                        else:
                            current_action = int(np.random.randint(env.action_space.n))
                    else:
                        current_action = apply_action_mask(q_vals, allow_mask)
                    action_timer = max(1, _sample_duration(action_duration_options))
                action_timer -= 1
                ...
                pos_tuple = _extract_position(next_info)
                if manager.update_progress(pos_tuple, float(reward)):
                    resume_latest = manager.resume_state_path()
                    if resume_latest and hasattr(env, "pyboy"):
                        try:
                            with open(resume_latest, "rb") as fh:
                                env.pyboy.load_state(BytesIO(fh.read()))  # type: ignore[attr-defined]
                            hidden = policy.init_hidden(1, device)
                        except Exception:
                            pass
                if telemetry_writer and telemetry_throttle is not None:
                    if time.monotonic() - last_telemetry_time >= telemetry_throttle:
                        coords = pos_tuple
                        telemetry_writer.enqueue(
                            TelemetryEvent(
                                ts=time.time(),
                                run=cfg.run_name,
                                actor_id=rank,
                ...
                if now - last_snapshot_time >= snapshot_interval_s:
                    try:
                        buf = BytesIO()
                        env.pyboy.save_state(buf)  # type: ignore[attr-defined]
                        manager.write_snapshot_bytes(buf.getvalue())
                        try:
                ...
                if getattr(cfg, "continuous_mode", False) and len(transitions) >= max(1, int(getattr(cfg, "segment_length", 2000))):
                    ...
                    if cfg.log_episodes_for_replay and log_dir:
                        try:
                            ensure_dir(log_dir)
                            seg_path = os.path.join(log_dir, f"{rank_prefix}actor{rank}_seg{segment_idx:05d}.npz")
                ...
                if cfg.log_episodes_for_replay and log_dir:
                    try:
                        ...
                        if getattr(cfg, "continuous_mode", False):
                            ...
                            ep_path = os.path.join(log_dir, f"{rank_prefix}actor{rank}_ep{episode:05d}.npz")
