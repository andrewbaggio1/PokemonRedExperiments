...
def _group_segments_by_actor(paths: List[str]) -> Dict[int, List[Tuple[int, str]]]:
    ...
    # Support both legacy and DDP-prefixed names: r<ddpRank>_actor<actorId>_segNNNNN.npz
    pat = re.compile(r"(?:r(\d+)_)?actor(\d+)_(?:seg|ep)(\d+)\.npz$")
    for p in paths:
        name = os.path.basename(p)
        m = pat.search(name)
        if not m:
            continue
        # m.group(1) may be ddp rank (optional); actor id is group(2); index is group(3)
        actor_id = int(m.group(2))
