...
from .config import ClusterConfig, load_config, save_config
...
def main():
    ...
    ep_queue: Queue = Queue(maxsize=num_actors * 2)
    stop_event = Event()
    ...
    logs_dir = os.path.join(run_dir, getattr(cfg.paths, "logs_dir", "logs"))
    ensure_dir(logs_dir)
    ...
    def _resolve_resume_checkpoint() -> Optional[str]:
        policy = getattr(cfg.resume, "resume_policy", "latest")
        specific = getattr(cfg.resume, "resume_from_id", None)
        if policy == "specific" and specific:
            if os.path.isabs(specific):
                return specific if os.path.exists(specific) else None
            candidate = os.path.join(checkpoint_dir, specific)
