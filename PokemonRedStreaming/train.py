...
from minimal_epsilon_setup import CatchPokemonReward, build_reward_modules, make_env_instance
...
def _cfg_namespace(cfg: dict) -> SimpleNamespace:
    return SimpleNamespace(
        ...
        emulation_speed=int(cfg.get("emulation_speed", 1)),
    )
...
def main():
    ...
    model = SimpleDQN(obs_shape, map_feat_dim, n_actions)
    ...
    if render_map:
        try:
            map_viz = MultiRouteMapVisualizer(num_envs, show_env_panels=show_env_maps)
    ...
    try:
        for ep in range(episodes):
            ...
            for idx, env in enumerate(envs):
                ...
                if render_map and gameplay_viz is None:
                ...
                if render_map and gameplay_viz:
